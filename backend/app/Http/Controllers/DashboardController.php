...
class DashboardController extends Controller
{
    ...
    public function getCardDoneProducts(Request $request)
    {
        ...
        $done_commissions[0] = count(Commission::select("id")
    ...
    public function getCardCommissionsVal(Request $request)
    {
        ...
        $commissions_val[0] = (float)Commission::selectRaw("SUM(indicator_commission + seller_commission  + operator_commission) as total")
            ->whereRaw("status = 'Aprovado Ups' AND
                month(CAST('" . $month_3 . "' as date)) = month(CAST(date_operator as date)) AND
                year(CAST('" . $month_3 . "' as date)) = year(CAST(date_operator as date))
            ")
            ->get()->toArray()[0]['total'];
        $commissions_val[1] = (float)Commission::selectRaw("SUM(indicator_commission + seller_commission  + operator_commission) as total")
            ->whereRaw("status = 'Aprovado Ups' AND
                month(CAST('" . $month_2 . "' as date)) = month(CAST(date_operator as date)) AND
                year(CAST('" . $month_2 . "' as date)) = year(CAST(date_operator as date))
            ")
            ->get()->toArray()[0]['total'];
        $commissions_val[2] = (float)Commission::selectRaw("SUM(indicator_commission + seller_commission  + operator_commission) as total")
            ->whereRaw("status = 'Aprovado Ups' AND
                month(CAST('" . $month_1 . "' as date)) = month(CAST(date_operator as date)) AND
                year(CAST('" . $month_1 . "' as date)) = year(CAST(date_operator as date))
            ")
            ->get()->toArray()[0]['total'];
        $commissions_val[3] = (float)Commission::selectRaw("SUM(indicator_commission + seller_commission  + operator_commission) as total")
            ->whereRaw("status = 'Aprovado Ups' AND
                month(CAST('" . $month . "' as date)) = month(CAST(date_operator as date)) AND
                year(CAST('" . $month . "' as date)) = year(CAST(date_operator as date))
            ")
    ...
    {
        $all_products = Commission::selectRaw("product, SUM(indicator_commission + seller_commission  + operator_commission) as qtt")
            ->where('status', 'Aprovado UPS')
            ->groupBy('product')->get()->toArray();
        $products = [];
        $qtts = [];
    ...
    {
        $all_indicators = Commission::selectRaw("indicator_id, SUM(indicator_commission) as total_indicator")
            ->where('status', 'Aprovado UPS')
            ->groupBy('indicator_id')
            ->with('indicator:id,full_name')
            ->get()->toArray();
        $all_sellers = Commission::selectRaw("seller_id, SUM(seller_commission) as total_seller")
            ->where('status', 'Aprovado UPS')
            ->groupBy('seller_id')
            ->with('seller:id,full_name')
            ->get()->toArray();
        $all_operators = Commission::selectRaw("operator_id, SUM(operator_commission) as total_operator")
            ->where('status', 'Aprovado UPS')
            ->groupBy('operator_id')
        ...
        foreach ($all_users_with_commission as $key => $user) {
            ...
            $all_users_with_commission[$key]['total'] = $all_users_with_commission[$key]['total_indicator'] + $all_users_with_commission[$key]['total_seller'] + $all_users_with_commission[$key]['total_operator'];
        }
        $all_users = [];
        $total = [];
        $indication = [];
        $selling = [];
        $operation = [];
        $ordering_column = array_column($all_users_with_commission, 'total');
        array_multisort($ordering_column, SORT_DESC, $all_users_with_commission);
        foreach ($all_users_with_commission as $key => $user) {
            array_push($all_users, $user['full_name']);
            array_push($total, $user['total']);
            array_push($indication, $user['total_indicator']);
            array_push($selling, $user['total_seller']);
            array_push($operation, $user['total_operator']);
        }
        $data = [["name" => "Indicação", "data" => $indication], ["name" => "Venda", "data" => $selling], ["name" => "Operação", "data" => $operation]];
        return response()->json([$all_users, $data, $total]);
    }
